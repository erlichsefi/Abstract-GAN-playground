...
# images size - [B_m,image_width,image_higth,image_channel]
def discriminator(images, reuse_variables=None):
    ...
    with tf.variable_scope(tf.get_variable_scope(), reuse=reuse_variables) as scope:
	   ...
	   #  [b_m,image_width/2,image_higth/2,l1_num_of_filters]
        ...
        l2_num_of_filters=64
        d_w2 = tf.get_variable('d_w2', [5, 5, l1_num_of_filters, l2_num_of_filters], initializer=tf.truncated_normal_initializer(stddev=0.02))
        d_b2 = tf.get_variable('d_b2', [l2_num_of_filters], initializer=tf.constant_initializer(0))
	...
	#  output  [b_m,image_width/4,image_higth/4,64]
        ...
        d=int(int(image_width/4)*int(image_higth/4)*64)
...
def generator(z, batch_size, random_dim):
    ...
    g1 = tf.nn.relu(g1)
    #  [batch_size,image_width,image_higth,image_channel]
...
Gz = generator(z_placeholder, batch_size, generator_input_size)
...
images_for_tensorboard = generator(z_placeholder, batch_size, generator_input_size)
...
for i in range(200):
    z_batch = np.random.normal(0, 1, size=[batch_size, generator_input_size])
    real_image_batch = next_batch(batch_size,input_images).reshape([-1, image_width, image_higth, image_channel])
    _, __, dLossReal, dLossFake = sess.run([d_trainer_real, d_trainer_fake, d_loss_real, d_loss_fake],
                                           {x_placeholder: real_image_batch, z_placeholder: z_batch})

# Train generator and discriminator together
for i in range(100000):
    real_image_batch = next_batch(batch_size,input_images)
    z_batch = np.random.normal(0, 1, size=[batch_size, generator_input_size])
